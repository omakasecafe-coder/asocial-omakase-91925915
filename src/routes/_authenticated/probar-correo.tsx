...
export const Route = createFileRoute("/_authenticated/probar-correo")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Envía manualmente los correos de resumen y de confirmación de pago para verificar la entrega." },
      { property: "og:title", content: "Probar correo · asocial café omakase" },
      { property: "og:description", content: "Envía manualmente los correos de resumen y de confirmación de pago." },
...
type TemplateKey = "reservation_confirmed" | "payment_confirmed";

const CARDS: { key: TemplateKey; title: string; description: string }[] = [
  {
    key: "reservation_confirmed",
    title: "Resumen de reserva",
    description: "El correo que recibe el cliente al reservar: datos de la reserva, medios de pago y aviso de enviar el comprobante por WhatsApp.",
  },
  {
    key: "payment_confirmed",
    title: "Confirmación de pago",
    description: "El correo que sale cuando se valida el pago: reserva confirmada con todos los datos.",
  },
];
  ...
  return (
    ...
    >
      ...
      <div className="mt-8 grid max-w-3xl gap-4 sm:grid-cols-2">
        {CARDS.map((card) => (
          <div key={card.key} className="rounded-xl border border-border bg-card p-5">
            ...
            >
              {mutation.isPending && mutation.variables === card.key ? "Enviando…" : "Enviar prueba"}
            </Button>
            {last?.key === card.key ? (
              <p className={`mt-3 text-xs ${last.ok ? "text-muted-foreground" : "text-destructive"}`}>
                {last.ok ? "Enviado correctamente." : `No se envió: ${last.reason ?? "motivo desconocido"}`}
              </p>
            ) : null}
          </div>
        ))}
      ...
      <p className="mt-8 max-w-xl text-xs leading-relaxed text-muted-foreground">
        Si el envío falla con “email_domain_not_configured”, el dominio de correo aún no terminó de verificarse.
      </p>
    </AdminShell>
  );
}
